...
func save_ct(model *gui.QStandardItemModel) {
	filename := getrootitemname(model) /// `projects.ct`
	f, err := os.Create(filename + ".ct")
	if err != nil {
		println(err.Error())
	}
	defer f.Close()
...
func saveforeach(model *gui.QStandardItemModel, parent *core.QModelIndex, w *orc.Writer, pid int64, uid *int64) {
	for i := 0; i < model.RowCount(parent); i++ {
		*uid += 1
		...
		err := w.Write(id, termid, id_parent) /// println(id, termid, id_parent)
		if err != nil {
			println(err.Error())
		}
