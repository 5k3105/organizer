...
func New_EditTree(window *widgets.QMainWindow) (*EditTree, *widgets.QVBoxLayout) {
	...
	header := gui.NewQStandardItem2(`Projects`)
	header.SetFont(font)
...
func edittree_ui(edittree *EditTree, window *widgets.QMainWindow) *widgets.QHBoxLayout {
	...
	deletebutton := widgets.NewQPushButton3(gui.NewQIcon5(":/icons/deletenode.png"), "   Delete", window)
	deletebutton.ConnectClicked(func(_ bool) { edittree.deletebutton_click() })
	...
	//viewbutton := widgets.NewQPushButton3(gui.NewQIcon5(":/icons/viewfile.png"), " View", window)
	//viewbutton.ConnectClicked(func(_ bool) { cm.viewbutton_click() })


	edittree.editbutton = editbutton

	hlayout.AddWidget(addbutton, 0, 0)
	hlayout.AddWidget(deletebutton, 0, 0)
...
func (edittree *EditTree) deletebutton_click() {
	edittree.DeleteNode(edittree.Im, edittree.CurrentIndex(), 0)
}
...
func (edittree *EditTree) DeleteNode(model *gui.QStandardItemModel, index *core.QModelIndex, pid int64) {

	root := model.ItemFromIndex(index)
	//println("model.RowCount(parent):  ", model.RowCount(index))
	for i := 0; i < model.RowCount(index); i++ {
		idx := model.Index(i, 0, index)
		item := model.ItemFromIndex(idx)
		root.RemoveRow(item.Row())

		/// delete from funcMap and io_map
		//println(pid)
		id := pid

		if model.HasChildren(idx) {
			edittree.DeleteNode(model, idx, id)
		}
	}
	...
	if !index.Parent().IsValid() {
		...
		root.Parent().RemoveRow(index.Row())
	}
